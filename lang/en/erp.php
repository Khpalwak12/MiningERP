<?php

return [
    'app_name' => 'Marble Quarry ERP',
    'nav' => [
        'dashboard' => 'Dashboard',
        'customers' => 'Customers',
        'shipments' => 'Marble Shipments',
        'payments' => 'Customer Payments',
        'sankari' => 'Sankari Stone Sales',
        'expenses' => 'Expenses',
        'employees' => 'Employees',
        'payroll' => 'Payroll',
        'inventory' => 'Inventory',
        'mine_assets' => 'Mine Assets',
        'machinery' => 'Machinery',
        'personal_accounts' => 'Personal Accounts',
        'contractor_royalty' => 'Contractor Royalty',
        'accounting' => 'Accounting',
        'financial_years' => 'Financial Years',
        'reports' => 'Reports',
        'users' => 'Users',
        'roles' => 'Roles',
        'backups' => 'Data Backup',
        'activity_logs' => 'Activity Logs',
        'profile' => 'Profile',
        'logout' => 'Log Out',
    ],
    'actions' => [
        'create' => 'Create',
        'edit' => 'Edit',
        'delete' => 'Delete',
        'save' => 'Save',
        'cancel' => 'Cancel',
        'search' => 'Search',
        'filter' => 'Filter',
        'export' => 'Export',
        'export_excel' => 'Export Excel',
        'export_pdf' => 'Export PDF',
        'view' => 'View',
        'print' => 'Print',
        'download' => 'Download',
        'column' => 'Actions',
        'back' => 'Back',
        'reset' => 'Reset',
    ],
    'status' => [
        'active' => 'Active',
        'inactive' => 'Inactive',
    ],
    'fields' => [
        'name' => 'Name',
        'owner_name' => 'Owner Name',
        'father_name' => 'Father Name',
        'phone' => 'Phone',
        'address' => 'Address',
        'status' => 'Status',
        'email' => 'Email',
        'password' => 'Password',
        'password_confirmation' => 'Confirm Password',
        'locale' => 'Language',
        'date' => 'Date',
        'amount' => 'Amount',
        'notes' => 'Notes',
        'description' => 'Description',
        'customer' => 'Customer',
        'employee' => 'Employee',
        'category' => 'Category',
        'subcategory' => 'Subcategory',
        'bill_number' => 'Bill Number',
        'truck_number' => 'Truck Number',
        'driver_name' => 'Driver Name',
        'quantity_ton' => 'Quantity (Ton)',
        'price_per_ton' => 'Price per Ton',
        'price_per_truck' => 'Price per Truck',
        'truck_count' => 'Truck Count',
        'subtotal' => 'Subtotal',
        'discount' => 'Discount',
        'total_amount' => 'Total Amount',
        'payment_method' => 'Payment Method',
        'receipt_number' => 'Receipt Number',
        'received_by' => 'Received By',
        'payment_type' => 'Payment Type',
        'cash_received' => 'Cash Received',
        'position' => 'Position',
        'salary' => 'Salary',
        'joining_date' => 'Joining Date',
        'registration_date' => 'Registration Date',
        'related_to' => 'Related To',
        'current_shamsi_date' => 'Current Shamsi Date',
        'employee_end_date' => 'End Date',
        'absence_days' => 'Absence Days',
        'absence_deduction' => 'Absence Deduction',
        'absence_date' => 'Absence Date',
        'days' => 'Days',
        'gross_earned_salary' => 'Gross Earned Salary',
        'months_worked' => 'Months Worked',
        'total_earned_salary' => 'Total Earned Salary',
        'total_paid_salary' => 'Total Paid Salary',
        'remaining_balance' => 'Remaining Balance',
        'overpaid_amount' => 'Overpaid Amount',
        'period_month' => 'Period Month',
        'sku' => 'SKU',
        'unit' => 'Unit',
        'min_stock' => 'Minimum Stock',
        'current_stock' => 'Current Stock',
        'movement_type' => 'Movement Type',
        'quantity' => 'Quantity',
        'reference' => 'Reference',
        'attachment' => 'Attachment',
        'marble_type' => 'Marble Type',
        'mine_type' => 'Mine Type',
        'stone_type' => 'Stone Type',
        'roles' => 'Roles',
        'permissions' => 'Permissions',
        'debit' => 'Debit',
        'credit' => 'Credit',
        'account' => 'Account',
        'entry_date' => 'Entry Date',
        'date_from' => 'From Date',
        'date_to' => 'To Date',
        'search' => 'Search',
        'created_at' => 'Created At',
        'created_by' => 'Created By',
        'financial_year' => 'Financial Year',
        'year_name' => 'Year Name',
        'start_date' => 'Start Date',
        'end_date' => 'End Date',
        'rate_per_ton' => 'Rate Per Ton',
        'truck_number' => 'Truck Number',
        'remarks' => 'Remarks',
        'type' => 'Type',
        'currency' => 'Currency',
    ],
    'payment_methods' => [
        'cash' => 'Cash',
        'bank_transfer' => 'Bank Transfer',
        'check' => 'Check',
        'other' => 'Other',
    ],
    'payment_types' => [
        'cash' => 'Cash',
        'credit' => 'Credit',
        'full_salary' => 'Full Salary',
        'advance' => 'Advance',
        'partial' => 'Partial',
    ],
    'movement_types' => [
        'in' => 'Stock In',
        'out' => 'Stock Out',
    ],
    'dashboard' => [
        'title' => 'Dashboard',
        'marble_sales' => 'Marble Sales',
        'marble_trucks' => 'Trucks',
        'marble_tons' => 'Tons',
        'sankari_sales' => 'Sankari Sales',
        'total_income' => 'Total Income',
        'payments_received' => 'Payments Received',
        'factory_payments_received' => 'Factory Payments Received',
        'total_expenses' => 'Total Expenses',
        'total_payroll' => 'Total Payroll',
        'net_profit' => 'Net Profit',
        'active_financial_year' => 'Active Financial Year',
        'active_customers' => 'Active Customers',
        'active_employees' => 'Active Employees',
        'low_stock_items' => 'Low Stock Items',
        'recent_shipments' => 'Recent Shipments',
        'recent_payments' => 'Recent Payments',
        'contractor_total_royalties' => 'Contractor Total Royalties',
        'contractor_payments_received' => 'Contractor Payments Received',
        'contractor_outstanding_balance' => 'Contractor Outstanding Balance',
        'contractor_salary_charges' => 'Shared Salary Owed',
        'contractor_expense_charges' => 'Contractor Expense Owed',
        'contractor_dispatches' => 'Dispatches',
        'contractor_tons' => 'Tons',
        'machinery_total_afn' => 'Total Machinery (AFN)',
        'machinery_total_usd' => 'Total Machinery (USD)',
        'income_expense_chart' => 'Income vs Expenses Trend',
    ],
    'customers' => [
        'title' => 'Customers',
        'create' => 'Add Customer',
        'edit' => 'Edit Customer',
        'show' => 'Customer Details',
        'created' => 'Customer created successfully.',
        'updated' => 'Customer updated successfully.',
        'deleted' => 'Customer deleted successfully.',
        'outstanding_balance' => 'Outstanding Balance',
        'total_sales' => 'Total Sales',
        'total_payments' => 'Total Payments',
    ],
    'shipments' => [
        'title' => 'Marble Shipments',
        'create' => 'Add Shipment',
        'edit' => 'Edit Shipment',
        'show' => 'Shipment Details',
        'created' => 'Shipment recorded successfully.',
        'updated' => 'Shipment updated successfully.',
        'deleted' => 'Shipment deleted successfully.',
        'optional_weight_price' => 'Weight and price per ton can be entered later when confirmed by the factory.',
        'pending_total' => 'Pending — enter weight and price to calculate total',
        'statuses' => [
            'completed' => 'Completed',
            'pending_weight' => 'Pending Weight',
            'pending_price' => 'Pending Price',
            'pending_both' => 'Pending Weight & Price',
            'pending' => 'Pending (All)',
        ],
    ],
    'payments' => [
        'title' => 'Customer Payments',
        'create' => 'Record Payment',
        'edit' => 'Edit Payment',
        'show' => 'Payment Details',
        'created' => 'Payment recorded successfully.',
        'updated' => 'Payment updated successfully.',
        'deleted' => 'Payment deleted successfully.',
    ],
    'sankari' => [
        'title' => 'Sankari Stone Sales',
        'create' => 'Add Sale',
        'edit' => 'Edit Sale',
        'show' => 'Sale Details',
        'created' => 'Sankari sale recorded successfully.',
        'updated' => 'Sankari sale updated successfully.',
        'deleted' => 'Sankari sale deleted successfully.',
        'discount_exceeds_subtotal' => 'Discount cannot exceed the subtotal.',
    ],
    'expenses' => [
        'title' => 'Expenses',
        'create' => 'Add Expense',
        'edit' => 'Edit Expense',
        'show' => 'Expense Details',
        'created' => 'Expense recorded successfully.',
        'updated' => 'Expense updated successfully.',
        'deleted' => 'Expense deleted successfully.',
        'for_contractor' => 'For Contractor Company',
        'for_contractor_hint' => 'You paid this expense; the contractor owes you back. It is tracked in the royalty ledger.',
    ],
    'expense_categories' => [
        'title' => 'Expense Categories',
        'create' => 'Add Category',
        'edit' => 'Edit Category',
        'created' => 'Category created successfully.',
        'updated' => 'Category updated successfully.',
        'deleted' => 'Category deleted successfully.',
        'name_en' => 'English Name',
        'name_ps' => 'Pashto Name',
        'in_use' => 'This category is used in expenses and cannot be deleted.',
    ],
    'mine_types' => [
        'title' => 'Mine Types',
        'create' => 'Add Mine Type',
        'edit' => 'Edit Mine Type',
        'created' => 'Mine type created successfully.',
        'updated' => 'Mine type updated successfully.',
        'deleted' => 'Mine type deleted successfully.',
        'name_en' => 'English Name',
        'name_ps' => 'Pashto Name',
        'in_use' => 'This mine type is used in shipments and cannot be deleted.',
    ],
    'stone_types' => [
        'title' => 'Stone Types',
        'create' => 'Add Stone Type',
        'edit' => 'Edit Stone Type',
        'created' => 'Stone type created successfully.',
        'updated' => 'Stone type updated successfully.',
        'deleted' => 'Stone type deleted successfully.',
        'name_en' => 'English Name',
        'name_ps' => 'Pashto Name',
        'in_use' => 'This stone type is used in shipments and cannot be deleted.',
    ],
    'financial_years' => [
        'title' => 'Financial Years',
        'create' => 'Add Financial Year',
        'edit' => 'Edit Financial Year',
        'show' => 'Financial Year Details',
        'close' => 'Close Year',
        'activate' => 'Activate Year',
        'created' => 'Financial year created successfully.',
        'updated' => 'Financial year updated successfully.',
        'closed' => 'Financial year closed successfully.',
        'activated' => 'Financial year activated successfully.',
        'active_label' => 'Active Financial Year',
        'confirm_close' => 'Are you sure you want to close this financial year? Transactions in this year will become read-only.',
        'all_years_name' => 'All Years',
        'all_years_mode' => 'All Years Mode',
        'current_mode_all' => 'Current Mode: All Years',
        'current_mode_year' => 'Current Mode: Financial Year :name',
        'all_years_read_only' => 'All Years mode is active. Please activate a specific financial year before creating new transactions.',
        'cannot_create_all_option' => 'The All Years option is managed by the system and cannot be created manually.',
        'cannot_close_all_option' => 'The All Years option cannot be closed. Activate a specific financial year instead.',
        'confirm_activate_all' => 'Are you sure you want to activate All Years mode? New transactions will be disabled and combined data from all years will be shown.',
        'confirm_activate' => 'Are you sure you want to make :name the active financial year?',
        'no_active_year' => 'No active financial year is configured. Please activate a financial year first.',
        'closed_year_locked' => 'This record belongs to a closed financial year and cannot be modified.',
        'already_closed' => 'This financial year is already closed.',
        'statuses' => [
            'active' => 'Active',
            'closed' => 'Closed',
        ],
    ],
    'employees' => [
        'title' => 'Employees',
        'create' => 'Add Employee',
        'edit' => 'Edit Employee',
        'show' => 'Employee Details',
        'created' => 'Employee created successfully.',
        'updated' => 'Employee updated successfully.',
        'deleted' => 'Employee deleted successfully.',
        'total_paid' => 'Total Paid',
        'remaining_salary' => 'Remaining Salary',
        'profile_details' => 'Profile Details',
        'shared_with_contractor' => 'Shared with Contractor',
        'contractor_salary_share' => 'Contractor Salary Share (%)',
        'shared_with_contractor_hint' => 'You pay full salary; the contractor share is tracked as receivable in the royalty ledger.',
        'end_date_hint' => 'Leave empty to calculate until today. Set this when the employee leaves so salary stops accruing.',
        'absences' => 'Absences',
        'add_absence' => 'Add Absence',
    ],
    'employee_absences' => [
        'created' => 'Absence recorded successfully.',
        'deleted' => 'Absence deleted successfully.',
    ],
    'payroll_statuses' => [
        'credit' => 'Employee Credit (Company Owes)',
        'settled' => 'Settled',
        'overpaid' => 'Overpaid',
    ],
    'payroll' => [
        'title' => 'Payroll Payments',
        'create' => 'Record Payroll Payment',
        'edit' => 'Edit Payroll Payment',
        'show' => 'Payroll Payment Details',
        'created' => 'Payroll payment recorded successfully.',
        'updated' => 'Payroll payment updated successfully.',
        'deleted' => 'Payroll payment deleted successfully.',
        'employee_summary' => 'Employee Salary Summary',
    ],
    'inventory' => [
        'title' => 'Inventory',
        'create' => 'Add Inventory Item',
        'edit' => 'Edit Inventory Item',
        'show' => 'Inventory Item Details',
        'created' => 'Inventory item created successfully.',
        'updated' => 'Inventory item updated successfully.',
        'deleted' => 'Inventory item deleted successfully.',
        'movement_created' => 'Inventory movement recorded successfully.',
        'movements' => 'Stock Movements',
        'add_movement' => 'Record Movement',
        'low_stock' => 'Low Stock',
    ],
    'mine_assets' => [
        'title' => 'Mine Assets',
        'create' => 'Add Asset',
        'edit' => 'Edit Asset',
        'show' => 'Asset Details',
        'created' => 'Asset recorded successfully.',
        'updated' => 'Asset updated successfully.',
        'deleted' => 'Asset deleted successfully.',
        'all_statuses' => 'All Statuses',
    ],
    'machinery' => [
        'title' => 'Machinery',
        'create' => 'Add Machinery',
        'edit' => 'Edit Machinery',
        'created' => 'Machinery record created successfully.',
        'updated' => 'Machinery record updated successfully.',
        'deleted' => 'Machinery record deleted successfully.',
        'total_afn' => 'Total in AFN',
        'total_usd' => 'Total in USD',
    ],
    'mine_asset_statuses' => [
        'usable' => 'Usable',
        'unusable' => 'Unusable (Retired)',
    ],
    'mine_asset_units' => [
        'piece' => 'Piece',
        'meter' => 'Meter',
        'packet' => 'Packet',
        'set' => 'Set',
        'ton' => 'Ton',
        'liter' => 'Liter',
        'kg' => 'Kg',
    ],
    'personal_accounts' => [
        'title' => 'Personal Accounts',
        'contacts' => 'Loans & Credit',
        'home_expenses' => 'Home Expenses',
        'contact_created' => 'Contact saved successfully.',
        'contact_updated' => 'Contact updated successfully.',
        'contact_deleted' => 'Contact deleted successfully.',
        'transaction_created' => 'Transaction recorded successfully.',
        'transaction_updated' => 'Transaction updated successfully.',
        'transaction_deleted' => 'Transaction deleted successfully.',
        'home_expense_created' => 'Home expense recorded successfully.',
        'home_expense_updated' => 'Home expense updated successfully.',
        'home_expense_deleted' => 'Home expense deleted successfully.',
        'create_contact' => 'Add Person/Shopkeeper',
        'edit_contact' => 'Edit Contact',
        'add_transaction' => 'Add Transaction',
        'edit_transaction' => 'Edit Transaction',
        'create_home_expense' => 'Add Home Expense',
        'edit_home_expense' => 'Edit Home Expense',
        'ledger' => 'Personal Ledger',
        'balance_afn' => 'Balance (AFN)',
        'balance_usd' => 'Balance (USD)',
        'total_credit_afn' => 'Total credit (AFN)',
        'total_payment_afn' => 'Total payments (AFN)',
        'total_credit_usd' => 'Total credit (USD)',
        'total_payment_usd' => 'Total payments (USD)',
        'contact_balances' => 'Contact balances',
        'running_balance' => 'Running Balance',
    ],
    'personal_contact_types' => [
        'person' => 'Person',
        'shopkeeper' => 'Shopkeeper',
    ],
    'personal_transaction_types' => [
        'credit' => 'Loan/Purchase (debt increases)',
        'payment' => 'Payment (debt decreases)',
    ],
    'currencies' => [
        'AFN' => 'AFN',
        'USD' => 'USD',
    ],
    'accounting' => [
        'title' => 'Journal Entries',
        'create' => 'Add Journal Entry',
        'edit' => 'Edit Journal Entry',
        'show' => 'Journal Entry Details',
        'created' => 'Journal entry created successfully.',
        'updated' => 'Journal entry updated successfully.',
        'deleted' => 'Journal entry deleted successfully.',
        'trial_balance' => 'Trial Balance',
        'unbalanced_entry' => 'Journal entry debits and credits must balance.',
        'lines' => 'Entry Lines',
    ],
    'reports' => [
        'title' => 'Reports',
        'sales' => 'Sales Report',
        'sankari' => 'Sankari Stone Sales Report',
        'payments' => 'Payments Report',
        'expenses' => 'Expenses Report',
        'payroll' => 'Payroll Report',
        'inventory' => 'Inventory Report',
        'mine_assets' => 'Mine Assets Report',
        'machinery' => 'Machinery Report',
        'customer_balances' => 'Customer Balances',
        'profit_loss' => 'Profit & Loss',
        'marble_sales' => 'Marble Sales',
        'sankari_sales' => 'Sankari Sales',
        'total_income' => 'Total Income',
        'total_payments' => 'Total Payments',
        'total_sales' => 'Total Sales',
        'operating_expenses' => 'Operating Expenses',
        'payroll_expenses' => 'Payroll Expenses',
        'total_expenses' => 'Total Expenses',
        'net_profit' => 'Net Profit',
        'outstanding_balance' => 'Outstanding Balance',
        'generated_at' => 'Generated At',
        'applied_filters' => 'Applied Filters',
        'low_stock_only' => 'Low Stock Only',
        'total_records' => 'Total Records',
        'totals' => 'Totals',
        'employee_salary_summary' => 'Employee Salary Summary',
        'payment_transactions' => 'Payment Transactions',
        'employees' => 'Employee Report',
        'daily_production' => 'Daily Production Report',
        'monthly_production' => 'Monthly Production Report',
        'contractor_production' => 'Contractor Production Report',
        'contractor_payments' => 'Contractor Payments Report',
        'contractor_expenses' => 'Contractor Expenses Report',
        'contractor_ledger' => 'Contractor Ledger Report',
        'contractor_royalty' => 'Contractor Royalty Revenue',
        'personal_ledger' => 'Personal Loans & Credit Report',
        'personal_home_expenses' => 'Home Expenses Report',
    ],
    'contractor_royalty' => [
        'title' => 'Contractor Royalty',
        'productions' => 'Production',
        'payments' => 'Payments',
        'ledger' => 'Royalty Ledger',
        'create_production' => 'Add Production',
        'edit_production' => 'Edit Production',
        'production_details' => 'Production Details',
        'create_payment' => 'Add Payment',
        'edit_payment' => 'Edit Payment',
        'payment_details' => 'Payment Details',
        'production_created' => 'Production record created successfully.',
        'production_updated' => 'Production record updated successfully.',
        'production_deleted' => 'Production record deleted successfully.',
        'payment_created' => 'Payment recorded successfully.',
        'payment_updated' => 'Payment updated successfully.',
        'payment_deleted' => 'Payment deleted successfully.',
        'total_royalty' => 'Total Royalty',
        'total_royalties' => 'Total Royalties',
        'total_payments_received' => 'Total Payments Received',
        'outstanding_balance' => 'Outstanding Balance',
        'royalty_entry' => 'Royalty',
        'payment_entry' => 'Payment',
        'salary_charge_entry' => 'Shared Salary',
        'salary_charge_for_employee' => 'Shared salary — :employee (:period)',
        'total_salary_charges' => 'Shared Salary Owed',
        'expense_charge_entry' => 'Contractor Expense',
        'expense_charge_for_expense' => 'Contractor expense — :description',
        'total_expense_charges' => 'Contractor Expense Owed',
        'total_receivable' => 'Total Receivable (Royalty + Salary + Expense)',
        'production_entry' => 'Production',
    ],
    'report_filters' => [
        'filter_by' => 'Filter By',
        'value' => 'Value',
        'shipment_number' => 'Shipment Number',
        'quantity' => 'Quantity',
        'price_per_ton' => 'Price Per Ton',
        'total_amount' => 'Total Amount',
        'created_by' => 'Created By',
        'sub_category' => 'Sub Category',
        'employee_name' => 'Employee Name',
        'paid_by' => 'Paid By',
        'item_name' => 'Item Name',
        'item_code' => 'Item Code',
        'total_tons' => 'Total Tons',
        'month' => 'Month',
        'revenue' => 'Revenue',
        'expense' => 'Expense',
        'net_profit' => 'Net Profit',
    ],
    'users' => [
        'title' => 'Users',
        'create' => 'Add User',
        'edit' => 'Edit User',
        'show' => 'User Details',
        'created' => 'User created successfully.',
        'updated' => 'User updated successfully.',
        'deleted' => 'User deleted successfully.',
        'cannot_delete_self' => 'You cannot delete your own account.',
    ],
    'roles' => [
        'title' => 'Roles',
        'create' => 'Add Role',
        'edit' => 'Edit Role',
        'show' => 'Role Details',
        'created' => 'Role created successfully.',
        'updated' => 'Role updated successfully.',
        'deleted' => 'Role deleted successfully.',
        'cannot_delete_super_admin' => 'The Super Admin role cannot be deleted.',
    ],
    'backup' => [
        'title' => 'Data Backup & Restore',
        'description' => 'Create full backups of the database and uploaded files, then restore them when needed.',
        'create' => 'Create Backup',
        'restore' => 'Restore',
        'restore_upload' => 'Restore from File',
        'download' => 'Download',
        'storage_path' => 'Backup folder',
        'database_driver' => 'Database type',
        'filename' => 'File name',
        'file_size' => 'Size',
        'created' => 'Backup created successfully.',
        'restored' => 'Data restored successfully from backup.',
        'deleted' => 'Backup deleted successfully.',
        'not_found' => 'Backup file was not found.',
        'invalid_archive' => 'The selected file is not a valid ERP backup.',
        'unsupported_driver' => 'This database type is not supported for backup.',
        'driver_mismatch' => 'This backup was created with a different database type.',
        'export_failed' => 'Database export failed.',
        'import_failed' => 'Database import failed.',
        'zip_failed' => 'Could not create the backup archive.',
        'confirm_restore' => 'This will replace all current data with the backup. Continue?',
        'confirm_delete' => 'Delete this backup file?',
        'no_backups' => 'No backups found yet.',
        'upload_hint' => 'Upload a .zip backup file created by this system.',
        'memory_not_supported' => 'In-memory databases cannot be backed up. Use a file-based SQLite or MySQL database.',
        'mysql_tools_missing' => 'MySQL backup tools were not found (:binary). Set ERP_MYSQL_BIN_PATH in .env to your MySQL bin folder, for example C:\\laragon\\bin\\mysql\\mysql-8.4.3-winx64\\bin',
    ],
    'activity_logs' => [
        'title' => 'Activity Logs',
        'subject' => 'Subject',
        'causer' => 'User',
        'event' => 'Event',
    ],
    'locale' => [
        'english' => 'English',
        'pashto' => 'Pashto',
        'updated' => 'Language updated.',
    ],
    'messages' => [
        'confirm_delete' => 'Are you sure you want to delete this record?',
        'no_records' => 'No records found.',
        'saved' => 'Saved successfully.',
    ],
    'validation' => [
        'shamsi_date' => 'The :attribute must be a valid Shamsi date (YYYY/MM/DD).',
    ],
];
