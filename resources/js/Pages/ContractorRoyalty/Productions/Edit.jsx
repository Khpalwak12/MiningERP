...
export default function Edit({ production }) {
    ...
    return (
        <ErpLayout>
            <Head title={t('contractor_royalty.edit_production')} />
            <FlashMessage />
            <h1 className="mb-6 text-center text-2xl font-bold text-slate-800">{t('contractor_royalty.edit_production')}</h1>
            <ContractorRoyaltyNav active="contractor-royalty.productions.index" />

            <form onSubmit={(e) => { e.preventDefault(); put(route('contractor-royalty.productions.update', p.id)); }} className="mx-auto w-full max-w-2xl space-y-5 rounded-xl border border-slate-200 bg-gradient-to-b from-white to-slate-50/80 p-6 shadow-md ring-1 ring-slate-900/5 sm:p-8">
