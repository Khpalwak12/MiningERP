...
export default function Index({ payments, filters }) {
    ...
    const list = resourceItems(payments);

    return (
        <ErpLayout>
            <Head title={t('contractor_royalty.payments')} />
            <FlashMessage />
            <PageHeader
                title={t('contractor_royalty.payments')}
                createRoute={canCreateTransactions && can('contractor-royalty.create') ? route('contractor-royalty.payments.create') : null}
                createLabel={t('contractor_royalty.create_payment')}
            />
            <ContractorRoyaltyNav active="contractor-royalty.payments.index" />

            <form onSubmit={(e) => {
                e.preventDefault();
                router.get(route('contractor-royalty.payments.index'), { search, date_from: dateFrom, date_to: dateTo }, { preserveState: true });
                        ...
                        {list.map((row) => (
                            <tr key={row.id}>
                                ...
                                <td className="px-4 py-3">
                                    <ActionButtons
                                        viewHref={route('contractor-royalty.payments.show', row.id)}
                                        editHref={!isTransactionReadOnly && can('contractor-royalty.edit') && !row.is_locked ? route('contractor-royalty.payments.edit', row.id) : null}
                                        onDelete={!isTransactionReadOnly && can('contractor-royalty.delete') && !row.is_locked ? () => router.delete(route('contractor-royalty.payments.destroy', row.id)) : null}
                                    />
                                </td>
                            </tr>
                        ))}
