...
export default function Edit({ employee }) {
    ...
    return (
        <ErpLayout>
            ...
            <FlashMessage />
            <h1 className="mb-6 text-center text-2xl font-bold text-slate-800">{t('employees.edit')}</h1>

            <form onSubmit={(ev) => { ev.preventDefault(); put(route('employees.update', e.id)); }} className="mx-auto w-full max-w-2xl space-y-5 rounded-xl border border-slate-200 bg-gradient-to-b from-white to-slate-50/80 p-6 shadow-md ring-1 ring-slate-900/5 sm:p-8">
                ...
                <div>
                    ...
                    <select className="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value={data.status} onChange={(ev) => setData('status', ev.target.value)}>
