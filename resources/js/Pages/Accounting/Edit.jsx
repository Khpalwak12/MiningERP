...
export default function Edit({ entry, accounts }) {
    ...
    return (
        <ErpLayout>
            ...
            <FlashMessage />
            <h1 className="mb-6 text-2xl font-bold">{t('accounting.edit')}</h1>

            <form onSubmit={(ev) => { ev.preventDefault(); put(route('journal-entries.update', entryData.id)); }} className="max-w-4xl space-y-4 rounded-lg bg-white p-6 shadow">
                ...
                <div><InputLabel value={t('fields.description')} /><textarea className="mt-1 block w-full rounded-md border-gray-300" rows="2" value={data.description} onChange={(ev) => setData('description', ev.target.value)} /></div>
                <div className="space-y-3">
                    <InputError message={errors.lines} />
                    {data.lines.map((line, index) => (
                        <div key={index} className="grid gap-2 rounded border p-3 sm:grid-cols-5">
                            <select className="rounded-md border-gray-300 sm:col-span-2" value={line.account_id} onChange={(ev) => updateLine(index, 'account_id', ev.target.value)}>
                                <option value="">--</option>
