...
export default function Payroll({ rows, filters }) {
            ...
            <div className="overflow-x-auto rounded-lg bg-white shadow">
                <table className="min-w-full text-sm">
                    ...
                    <tbody className="divide-y">
                        {list.map((r) => (
                            <tr key={r.id}><td className="px-4 py-3">{r.payment_date}</td><td className="px-4 py-3">{r.employee?.name}</td><td className="px-4 py-3">{formatCurrency(r.amount)}</td><td className="px-4 py-3">{r.payment_type}</td></tr>
                        ))}
                    </tbody>
                </table>
            </div>
