...
export default function ContractorExpenses({ rows, filters, expenseCategories, total }) {
    const { t } = useTranslation();
    const list = resourceItems(rows);

    return (
        <ErpLayout>
            <Head title={t('reports.contractor_expenses')} />
            <FlashMessage />
            <div className="mb-6 flex items-center justify-between">
                <h1 className="text-2xl font-bold">{t('reports.contractor_expenses')}</h1>
                <Link href={route('reports.index')} className="text-sm text-indigo-600 hover:underline">{t('actions.back')}</Link>
            </div>

            <ReportFilterForm
                reportType="contractor-expenses"
                routeName="reports.contractor-expenses"
                filters={filters}
                exportType="contractor-expenses"
                lookups={{ expenseCategories }}
            />

            <div className="mb-4">
                <StatCard title={t('fields.total_amount')} value={formatCurrency(total)} color="amber" />
            </div>
