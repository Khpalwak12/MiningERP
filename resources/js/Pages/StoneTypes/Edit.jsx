...
export default function Edit({ stoneType }) {
    ...
    return (
        <ErpLayout>
            ...
            <FlashMessage />
            <h1 className="mb-2 text-center text-2xl font-bold text-slate-800">{t('stone_types.edit')}</h1>
            <ShipmentsNav active="stone-types.index" />

            <form onSubmit={(e) => { e.preventDefault(); put(route('stone-types.update', m.id)); }} className="mx-auto w-full max-w-2xl space-y-5 rounded-xl border border-slate-200 bg-gradient-to-b from-white to-slate-50/80 p-6 shadow-md ring-1 ring-slate-900/5 sm:p-8">
                <div>
